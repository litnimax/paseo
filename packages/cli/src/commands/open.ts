...
function findDesktopApp(): string | null {
  if (process.platform === "darwin") {
    const candidates = [
      "/Applications/Odusphere.app",
      path.join(homedir(), "Applications", "Odusphere.app"),
    ];
  ...
  if (process.platform === "linux") {
    const candidates = [
      "/usr/bin/Odusphere",
      "/opt/Odusphere/Odusphere",
      path.join(homedir(), "Applications", "Odusphere.AppImage"),
    ];
  ...
  if (process.platform === "win32") {
    ...
    const candidate = path.join(localAppData, "Programs", "Odusphere", "Odusphere.exe");
...
function launchDesktop(args: string[]): void {
  if (process.env.PASEO_DESKTOP_CLI === "1") {
    throw new Error("Cannot open Odusphere Desktop while running in desktop CLI passthrough mode.");
  }

  const desktopApp = findDesktopApp();
  if (!desktopApp) {
    throw new Error(
      "Odusphere desktop app not found. Install it from https://github.com/litnimax/paseo/releases",
    );
  }
