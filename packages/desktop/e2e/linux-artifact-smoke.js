...
async function main() {
  ...
  try {
    ...
    const helper = fs.statSync("/opt/Odusphere/chrome-sandbox");
    ...
    await smokePackagedDesktopApp({ appPath: "/opt/Odusphere", expectedSandbox: true });
    ...
    const desktopEntry = fs.readFileSync(path.join(appDir, "Odusphere.desktop"), "utf8");
    ...
    const appPath = fs.existsSync(path.join(tarDir, "Odusphere"))
      ? tarDir
