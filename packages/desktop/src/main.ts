...
const APP_NAME = process.env.PASEO_TEST_APP_NAME?.trim() || "Odusphere";
...
if (forcedUserDataDir) {
  ...
  try {
    ...
    if (isWorktree) {
      app.setPath("userData", path.join(app.getPath("appData"), `Odusphere-${devWorktreeName}`));
...
if (process.platform === "linux") {
  ...
  app.setDesktopName("Odusphere.desktop");
  if (!app.commandLine.hasSwitch("class")) app.commandLine.appendSwitch("class", "Odusphere");
