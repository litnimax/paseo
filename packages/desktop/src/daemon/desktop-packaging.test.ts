...
function createFakeMacBundle(options: { includeHelper: boolean }): {
  ...
  const appPath = join(root, "Odusphere.app");
  const contentsPath = join(appPath, "Contents");
  const resourcesPath = join(contentsPath, "Resources");
  const shimPath = join(resourcesPath, "bin", "paseo");
  const mainPath = join(contentsPath, "MacOS", "Odusphere");
  const helperPath = join(
    contentsPath,
    "Frameworks",
    "Odusphere Helper.app",
    "Contents",
    "MacOS",
    "Odusphere Helper",
  );
...
describe("desktop packaging", () => {
  ...
  it("registers Odusphere agent links with the operating system", () => {
    const config = readFileSync(join(packageRoot, "electron-builder.yml"), "utf8");

    expect(config).toContain("name: Odusphere agent link");
    expect(config).toContain("- paseo");
  });

  it("publishes updates from the Odusphere fork and bundles legal notices", () => {
    const config = readFileSync(join(packageRoot, "electron-builder.yml"), "utf8");

    expect(config).toContain("owner: litnimax");
    expect(config).toContain("repo: paseo");
    expect(config).toContain("from: ../../LICENSE");
    expect(config).toContain("from: ../../NOTICE");
  });
  ...
  it("fails packaged macOS CLI startup when Helper is missing", () => {
    ...
    try {
      ...
      expect(result.stderr).toContain("Bundled Odusphere Helper executable not found");
      expect(result.stdout).not.toContain("main-executable");
    } finally {
      rmSync(bundle.root, { recursive: true, force: true });
    }
