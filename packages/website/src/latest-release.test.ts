...
function release({
  ...
  return {
    tag_name: tag,
    assets: hasApk ? [{ name: `odusphere-${tag}-android.apk` }] : [],
    prerelease,
    draft: false,
  };
}
...
function desktopRelease({
  ...
  return {
    tag_name: `v${version}`,
    assets: [
      { name: `Odusphere-${version}-arm64.dmg` },
      { name: "Odusphere-x86_64.AppImage" },
      { name: `Odusphere-Setup-${version}-x64.exe` },
      { name: `Odusphere-Setup-${version}-arm64.exe` },
    ],
    prerelease,
    draft,
  };
...
describe("selectReleaseChannels", () => {
  it("offers the newest prerelease when it leads stable", () => {
    ...
    expect(channels.beta?.windowsArm64Asset).toBe("Odusphere-Setup-0.3.0-beta.2-arm64.exe");
  });
