...
Start the `io.github.litnimax.odusphere.debug` dev client against the intended Metro server first;
...
The scripts assume a development build with package id `io.github.litnimax.odusphere.debug`, an
...
```bash
PASEO_MAESTRO_APP_ID=io.github.litnimax.odusphere.debug
