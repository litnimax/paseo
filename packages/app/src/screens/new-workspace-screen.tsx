...
/**
 * `worktree.baseBranch` from the project's paseo.json, or null when unset. Shares its cache key
 * with the project settings screen, so a base branch saved there lands here too.
 */
function useConfiguredBaseBranch(input: {
  serverId: string;
  cwd: string | null;
  clientReady: boolean;
  getClient: () => NonNullable<ReturnType<typeof useHostRuntimeClient>>;
}): string | null {
  const { cwd, getClient } = input;
  const { data } = useQuery({
    queryKey: ["project-config", input.serverId, cwd],
    queryFn: async () => {
      if (!cwd) {
        throw new Error("Choose a project");
      }
      return getClient().readProjectConfig(cwd);
    },
    enabled: input.clientReady && cwd !== null,
    retry: false,
  });

  return useMemo(() => {
    if (!data?.ok) return null;
    const baseBranch = data.config?.worktree?.baseBranch;
...
/**
 * The base a new workspace branches off when the user picked nothing: `worktree.baseBranch`
 * from paseo.json first, otherwise whatever the source checkout is on.
 */
function resolveDefaultBasePickerItem(input: {
  configuredBaseBranch: string | null;
  status: BaseRefCheckoutStatus;
}): PickerItem | null {
  if (input.configuredBaseBranch) {
    return {
      kind: "branch",
      name: input.configuredBaseBranch,
      refName: `refs/heads/${input.configuredBaseBranch}`,
      accessibilityLabel: `${input.configuredBaseBranch}, project base branch`,
    };
  }
  return defaultBasePickerItem(input.status);
}

/** The picker row the ref trigger shows: an explicit pick, else the project default base. */
function resolveBasePickerItem(input: {
  selectedItem: PickerItem | null;
  configuredBaseBranch: string | null;
  status: BaseRefCheckoutStatus | null;
}): PickerItem | null {
  if (input.selectedItem) return input.selectedItem;
  if (!input.status) return null;
  return resolveDefaultBasePickerItem({
...
export function NewWorkspaceScreen({
  ...
  const configuredBaseBranch = useConfiguredBaseBranch({
    serverId: selectedServerId,
    cwd: selectedSourceDirectory,
    clientReady,
  ...
  const baseItem = useMemo(
    () => resolveBasePickerItem({ selectedItem, configuredBaseBranch, status: checkoutStatus }),
  ...
  const ensureWorkspace = useCallback(
    async (input: {
      ...
      const checkoutRequest = checkoutStatusForCreate
        ? pickerItemToCheckoutRequest(
            resolveBasePickerItem({
              selectedItem,
              configuredBaseBranch,
              status: checkoutStatusForCreate,
            }),
          )
        : undefined;
